...
mkdir -p "${root_dir}/product_v3/"
cp -u "${root_dir}/product.jsonl" "${root_dir}/product_v3/product.jsonl"

python -m pyserini.index -collection JsonCollection \
 -generator DefaultLuceneDocumentGenerator \
 -threads 1 -input "${root_dir}/product_v3/" \
...
python -m pyserini.search --topics "${root_dir}/all_entities.tsv"  \
        --index "${root_dir}/bm25_index/" \
        --output "${root_dir}/runs/bm25.all.run" \
