...
def get_args():
    parser = HfArgumentParser((JointDataTrainArguments, ModelArguments))
...
def train(args, model_args):
    ...
    if args.distributed:
        ...
        train_dataset = JointTripleDataset.create_from_triple_file(args.queries_path, args.collection_path, args.collection_path,
                                                              args.training_path, tokenizer,
    ...
    for epoch_idx, epoch in enumerate(trange(start_epoch, args.num_train_epochs, desc="Epoch")):
        for step, batch in enumerate(tqdm(train_dataloader, desc="query_epoch", disable=args.local_rank >=1, total=len(train_dataloader))):
            ...
            with amp.autocast(enabled=args.use_fp16):
                loss = model(batch["query"], batch["passages"])
            if args.use_fp16:
