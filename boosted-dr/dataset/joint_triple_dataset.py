...
class JointTripleDataset(torch.utils.data.Dataset):
    def __init__(self, qid_to_query, pid_to_passage, aid_to_title, train_examples, 
                        tokenizer, max_query_len, max_passage_len):
        super(JointTripleDataset, self).__init__()
        self.qid_to_query = qid_to_query
        self.pid_to_passage = pid_to_passage
        self.aid_to_title = aid_to_title
    ...
    def __getitem__(self, idx):
        ...
        qid, relpid, negpid, tag = example
        
        if tag == "search":           
            query = self.qid_to_query[qid]
        elif tag == "rec":
            query = self.aid_to_title[qid]
        else:
            raise ValueError("tag = {} not exists".format(tag))
    ...
    @classmethod
    def create_from_triple_file(cls, queries_path, passages_path, title_path, training_path, tokenizer, max_query_len, max_passage_len, 
                                    rank=-1, nranks=None):
        if rank != -1:
        ...
        with open(title_path) as fin:
            for line in fin:
                array = line.strip().split("\t")
                assert len(array) == 2
                pid, title = int(array[0]), array[1]
                aid_to_title[pid] = title

        if rank == -1:
            train_examples = []
            with open(training_path, "r") as fin:
                for line in fin:
                    qid, relpid, negpid, tag = line.rstrip().split("\t")
                    qid, relpid, negpid = int(qid), int(relpid), int(negpid)
                    
                    train_examples.append((qid, relpid, negpid, tag))
        else:
            train_examples = []
            with open(training_path, "r") as fin:
                for line_idx, line in enumerate(fin):
                    if line_idx % nranks == rank:
                        ...
                        train_examples.append((qid, relpid, negpid, tag))
        return cls(qid_to_query, pid_to_passage, aid_to_title, train_examples, tokenizer, max_query_len, max_passage_len)
...
if __name__ == "__main__":
    ...
    dataset = JointTripleDataset.create_from_triple_file(queries_path, passages_path, title_path, training_path, tokenizer, max_query_len=30, max_passage_len=64)
