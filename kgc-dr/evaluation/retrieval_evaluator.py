...
class RankingEvaluator():
    ...
    def _get_qid_to_ranklist(self, ranking_path):
        qid_to_ranklist = {}
        with open(ranking_path, "r") as fin:
            for line in fin:
                array = line.strip().split('\t')
                ...
                if qid not in qid_to_ranklist:
                    ...
                    qid_to_ranklist[qid] += [pid]
        return qid_to_ranklist

    def get_low_performance_queries(self, ranking_path, output_path, cutoff=10):
        qid_to_ranklist = self._get_qid_to_ranklist(ranking_path)
        qrels = self.qid_to_relevant_data

        evaluated_queries = 0
        bad_queries = 0
        bad_qid_info = []
        for i, (qid, pids) in enumerate(qid_to_ranklist.items()):
            if i == 0:
                max_rank = len(qid_to_ranklist[qid])
            if qid not in qrels:
                continue
            evaluated_queries += 1
            relevant_ids = np.array(list(qrels[qid].keys()))
            relevant_mask = np.in1d(pids, relevant_ids)
            if np.any(relevant_mask):
                ranks = np.arange(1,relevant_mask.shape[0]+1)[relevant_mask]
                first_rank = ranks[0]
                if first_rank > cutoff:
                    bad_qid_info.append({"qid": qid, "rel_pids": relevant_ids.tolist(), "rank": int(first_rank)})
            else:
                bad_qid_info.append({"qid": qid, "rel_pids": relevant_ids.tolist(), "rank": int(max_rank)})
        
        with open(output_path, "w") as fout:
            fout.write(ujson.dumps({"evaluated_queries": evaluated_queries, "bad_queries": len(bad_qid_info), "cutoff": cutoff}) + "\n")
            for info in bad_qid_info:
                #print(info)
                fout.write(ujson.dumps(info) + "\n")
            

if __name__ == "__main__":
    #sys.path += ["../"]
    parser = argparse.ArgumentParser()
    parser.add_argument("--qrels_path", default="", type=str)
    parser.add_argument("--ranking_path", default="", type=str)
