...
from dataset import CENwayDataset
...
from arguments import CEModelArguments, CETrainArguments
...
def train(args, model_args):
    ...
    if args.distributed:
        ...
        if args.label_mode in ["1"]:
            ...
            train_dataset = CENwayDataset.create_from_relT_most_semi_hard_file(args.queries_path, args.collection_path, args.training_path, tokenizer,
            ...
            train_dataset = CENwayDataset.create_from_10relT_20neg_file(args.queries_path, args.collection_path, args.training_path, tokenizer, 
                                                    max_length=args.max_length,
                                                    label_mode=args.label_mode, rank=args.local_rank, nranks=args.nranks)
            train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size//args.nranks, shuffle=True, 
                                                    num_workers=1,collate_fn=train_dataset.collate_fn, drop_last=True)
        elif args.label_mode in ["5", "10"]:
            train_dataset = CENwayDataset.create_from_20relT_10neg_file(args.queries_path, args.collection_path, args.training_path, tokenizer, 
                                                    max_length=args.max_length,
                                                    label_mode=args.label_mode, rank=args.local_rank, nranks=args.nranks)
            train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size//args.nranks, shuffle=True, 
                                                    num_workers=1,collate_fn=train_dataset.collate_fn, drop_last=True)
        elif args.label_mode == "6":
            train_dataset = CENwayDataset.create_from_30relT_file(args.queries_path, args.collection_path, args.training_path, tokenizer, 
                                                    max_length=args.max_length,
                                                    label_mode=args.label_mode, rank=args.local_rank, nranks=args.nranks)
            train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size//args.nranks, shuffle=True, 
                                                    num_workers=1,collate_fn=train_dataset.collate_fn, drop_last=True)
        elif args.label_mode in ["7", "8"]:
            train_dataset = CENwayDataset.create_from_5relT_25neg_file(args.queries_path, args.collection_path, args.training_path, tokenizer, 
                                                max_length=args.max_length,
                                                label_mode=args.label_mode, rank=args.local_rank, nranks=args.nranks)
            train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size//args.nranks, shuffle=True, 
                                                    num_workers=1,collate_fn=train_dataset.collate_fn, drop_last=True)
        ...
        if args.label_mode == "1":
            train_dataset = CENwayDataset.create_from_json_line_file(args.queries_path, args.collection_path, args.training_path, tokenizer,
                                                            max_length=args.max_length,
    ...
    if args.model_pretrained_path: 
        ...
        model = CrossEncoder.from_pretrained(args.model_pretrained_path, model_args)
    else:
    ...
    for epoch_idx, epoch in enumerate(trange(start_epoch, args.num_train_epochs, desc="Epoch")):
        for step, batch in enumerate(tqdm(train_dataloader, desc="query_epoch", disable=args.local_rank >=1, total=len(train_dataloader))):
            ...
            with amp.autocast(enabled=args.use_fp16):
                pred_logits = model(batch["query_passages"]) # [bz * nway]
                pred_logits = pred_logits.view(batch["labels"].size()) # [bz, nway]
